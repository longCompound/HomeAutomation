...
{
    return [(NSArray *)columnRectsInSection[sectionIdx][columnIdx] count];
}
...
- (CGFloat)lastItemOffsetInColumn:(NSInteger)columnIdx inSection:(NSInteger)sectionIdx
{
    ...
    if (itemsInColumn.count == 0) {
        if([(NSArray *)headerFooterItemAttributes[UICollectionElementKindSectionHeader] count] > sectionIdx){
...
- (NSInteger)preferredColumnIndexInSection:(NSInteger)sectionIdx
{
    ...
    for (NSUInteger columnIdx = 0; columnIdx < [(NSArray *)columnRectsInSection[sectionIdx] count]; ++columnIdx) {
...
- (NSArray *)searchVisibleLayoutAttributesInRect:(CGRect)rect {
    ...
    [visibleSections enumerateIndexesUsingBlock:^(NSUInteger sectionIdx, BOOL *stop) {
        ...
        //# footer
        if([(NSArray *)headerFooterItemAttributes[UICollectionElementKindSectionFooter] count] > sectionIdx){
