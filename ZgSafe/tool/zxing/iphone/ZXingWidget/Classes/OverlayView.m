...
- (id) initWithFrame:(CGRect)theFrame cancelEnabled:(BOOL)isCancelEnabled oneDMode:(BOOL)isOneDModeEnabled showLicense:(BOOL)showLicenseButton {
    ...
    if( self ) {
        ...
        label = [[UILabel alloc] initWithFrame:CGRectMake(0, scan_y, self.frame.size.width, 2)];
...
- (void)drawRect:(CGRect)rect {
    ...
    if (displayedMessage == nil) {
        //    self.displayedMessage = @"请将条码置于取景框内扫描";
...
//    CGFloat drY = 210.0f;
//    CGContextMoveToPoint(c, 0, drY);
//    CGContextAddLineToPoint(c, self.frame.size.width, drY);
//    CGContextStrokePath(c);
	...
	if (oneDMode) {
        ...
        CGRect displayRect = CGRectMake((rect.size.width - displaySize.width) / 2 , cropRect.origin.y - displaySize.height - 20, displaySize.width, displaySize.height);
...
- (void)layoutSubviews {
    ...
    if (![label.layer animationForKey:@"move"]) {
        ...
        moveAnimation.toValue = [NSNumber numberWithFloat:222];
        moveAnimation.duration = 3.0f;
        moveAnimation.repeatCount = FLT_MAX;
        moveAnimation.autoreverses = YES;
        [label.layer addAnimation:moveAnimation forKey:@"move"];
    }
}
