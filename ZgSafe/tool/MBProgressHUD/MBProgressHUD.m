...
- (id)initWithSelfView:(UIView *)view
{
    NSParameterAssert(view);
    // Let's check if the view is nil (this is a common error when using the windw initializer above)
    if (!view) {
...
- (void)layoutSubviews {
    ...
    if (_isFixedFrame) {
        ...
        if (nil != self.labelText) {
            // Get size of label text
            //            CGSize dims = [self.labelText sizeWithFont:self.labelFont];
            CGSize dims = [self catulateTextHeigh:self.labelText withWidth:frame.size.width - 2 * MARGIN];
            // Compute label dimensions based on font metrics if size is larger than max then clip the label width
            float lHeight = dims.height;
            float lWidth;
            ...
            // Move indicator to make room for the label
            indFrame.origin.y -= (floor(lHeight / 2 + PADDING / 2));
            indFrame.size.height -= mode == MBProgressHUDModeIndeterminate ? 0 : PADDING;
            indicator.frame = indFrame;
            ...
            if (nil != self.detailsLabelText) {
                // Get size of label text
                dims = [self catulateTextHeigh:self.detailsLabelText withWidth:frame.size.width - 4 * MARGIN];
...
-(CGSize)catulateTextHeigh:(NSString*)text withWidth:(CGFloat)theWidth
{
    NSParameterAssert(text);
    if (!text || ![text isKindOfClass:[NSString class]]) {
        NSAssert(NO, @"the text is not a string.");
        return CGSizeMake(theWidth, 80);
    }
    
    CGSize maxSize = CGSizeMake(theWidth, CGFLOAT_MAX);
    NSMutableParagraphStyle *paragraphStyle = [[NSParagraphStyle defaultParagraphStyle] mutableCopy];
    paragraphStyle.lineBreakMode = NSLineBreakByWordWrapping;
    CGSize dims = [text boundingRectWithSize:maxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.labelFont,NSParagraphStyleAttributeName:paragraphStyle} context:nil].size;
    return dims;
}
